...
public class Orientacao {
	...
	public boolean read () {
		...
		try {
			conn.conectarBD();
			String sql = "select palavrachave, titulo, texto from orientacao where id = ?";
			PreparedStatement stmt = conn.getConector().prepareStatement(sql);
			stmt.setInt(1, id);
			ResultSet rs = stmt.executeQuery();
			//verificação se tornou registros
			if(rs.next()) {
	...
	public DefaultTableModel readByKeyWord (String value) {
		DefaultTableModel ret = null;
		
		try {
			conn.conectarBD();
			String sql = "select id, palavrachave, titulo, texto from orientacao where palavrachave like ? order by id";
			PreparedStatement stmt = conn.getConector().prepareStatement(sql);
			stmt.setString(1, "%"+value+"%");
			ResultSet rs = stmt.executeQuery();
			ResultSetMetaData rsm = stmt.getMetaData();
			//montando o vetor dos titulos da tabela
			...
			//criando o defaultablemodel com os titulos
...
DefaultTableModel ret = null;
		...
		try {
			conn.conectarBD();
			String sql = "select id, palavrachave, titulo, texto from orientacao order by id";
			PreparedStatement stmt = conn.getConector().prepareStatement(sql);
			ResultSet rs = stmt.executeQuery();
			ResultSetMetaData rsm = stmt.getMetaData();
			//montando o vetor dos titulos da tabela
