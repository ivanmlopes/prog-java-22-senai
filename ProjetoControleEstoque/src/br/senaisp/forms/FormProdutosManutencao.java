...
public class FormProdutosManutencao extends JFrame {
	...
	public FormProdutosManutencao() {
		...
		nmFmtPi.setMaximum(9999999999999.99);
	...
	public void setTipoOperacao(int tipoOperacao) {
		...
		//fica habilitado só quando for inclusao ou alteração
		...
		public void actionPerformed(ActionEvent e) {
			...
			if(btn.getText().equals("Confirmar")) {
				...
				if (tipoOperacao==1 || tipoOperacao == 3) {
					...
					if(JOptionPane.showConfirmDialog(null, "Confirma exclusaõ?", "Confirmação", JOptionPane.OK_CANCEL_OPTION) == JOptionPane.OK_OPTION) {
						retAcao = prod.delete();
					}else {
						retAcao = true;    
					}
				...
				if (!retAcao) {
					JOptionPane.showMessageDialog(null, "Ocorreu um erro na operação! Erro: " + prod.getMsgErro());
				}
